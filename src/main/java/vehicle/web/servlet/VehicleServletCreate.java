...
public class VehicleServletCreate extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		form.setVehicleType(info.get(0));
		form.setVehicleEmissionType(info.get(1));
		form.setGasMileage(Float.parseFloat(info.get(2)));
		form.setVehicleCity(info.get(3));
		form.setVehicleDate(info.get(4));
	
		try {
			service.createVehicle(form);
